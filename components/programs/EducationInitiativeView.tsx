...
export default function EducationInitiativeView({ program }: { program?: ProgramContentData }) {
  const title = program?.title || 'Empowering young minds.';
  const subtitle = program?.subtitle || 'Building future leaders.';
  const badge = program?.badge || 'Education initiative';
  const heroImage = program?.heroImage || '/isssa-education-program-v2.png';
  const overviewP1 =
    program?.overviewP1 ||
  ...
  const approachTitle = program?.approachTitle || 'A connected model for stronger schools';
  const approachDesc =
  ...
  const skills = program?.skills?.length
    ? program.skills
    : [
  ...
  const involvement = program?.involvement?.length
    ? program.involvement
    : [
        ...
        <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="max-w-4xl space-y-6">
            <p className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.2em] text-accent">
              <BookOpen className="h-4 w-4" aria-hidden="true" /> {badge}
            </p>
            <h1 className="max-w-3xl font-serif text-4xl font-bold tracking-tight sm:text-6xl">
              {title} <span className="font-normal italic text-accent">{subtitle}</span>
            </h1>
      ...
      <section className="bg-[#f4efe3]">
        <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8 lg:py-20">
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-5">
            {involvement.map((inv) => (
              <article key={inv.title} className="bg-white p-5 shadow-sm rounded-2xl">
                <Users className="mb-5 h-6 w-6 text-primary" aria-hidden="true" />
                <h3 className="mb-2 font-serif text-lg font-bold text-primary">{inv.title}</h3>
                <p className="text-sm leading-6 text-neutral-600">{inv.description}</p>
              </article>
            ))}
          </div>
