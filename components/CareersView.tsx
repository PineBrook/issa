              ...
              {jobs.map((job) => (
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                      <span className="text-xs font-sans text-neutral-600 font-bold uppercase tracking-wider">{job.dept}</span>
                  ...
                  <div className="flex flex-wrap gap-4 text-sm text-neutral-600 font-sans font-medium mt-4">
                    <span className="flex items-center gap-1.5"><MapPin className="w-4 h-4 text-rust" /> {job.location}</span>
                  </div>
                  ...
                  {activeJobId === job.id ? (
                    <div className="mt-6 pt-6 border-t border-neutral-100 space-y-4 animate-fade-in">
                      <h4 className="text-sm font-sans uppercase tracking-wider text-neutral-800 font-bold">Requirements:</h4>
                      <ul className="space-y-2.5">
                        {job.reqs.map((req, idx) => (
                          <li key={idx} className="text-sm text-neutral-700 flex items-start gap-2.5 font-sans">
                            <span className="w-2 h-2 bg-rust rounded-full shrink-0 mt-1.5"></span>
                            <span>{req}</span>
                          </li>
                        ))}
                      </ul>
            ...
            <div className="bg-white rounded-3xl p-8 border border-neutral-200/80 shadow-md">
              {submitted ? (
                ...
                <form onSubmit={handleApplySubmit} className="space-y-6">
                  <div className="space-y-2">
                    <span className="text-xs font-sans uppercase tracking-wider text-neutral-600 block font-bold">Join the cohort</span>
                    <h3 className="text-2xl font-serif font-bold text-primary">Apply Online</h3>
                  </div>

                  <div className="space-y-4">
                    <div className="space-y-1.5">
                      <label className="text-xs font-sans uppercase tracking-wider text-neutral-700 block font-bold">Full Name</label>
                      <input 
                        type="text" 
                        required
                        value={formData.name}
                        ...
                        className="w-full bg-neutral-50 border border-neutral-300 rounded-xl px-4 py-3 text-sm text-neutral-800 focus:outline-none focus:border-primary focus:bg-white transition-all font-sans"
                      />
                    </div>

                    <div className="space-y-1.5">
                      <label className="text-xs font-sans uppercase tracking-wider text-neutral-700 block font-bold">Email Address</label>
                      <input 
                        type="email" 
                        required
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        <label className="text-xs font-sans uppercase tracking-wider text-neutral-700 block font-bold">Target Role</label>
                        <select 
                          value={formData.role}
                          onChange={(e) => setFormData({ ...formData, role: e.target.value })}
                          className="w-full bg-neutral-50 border border-neutral-300 rounded-xl px-3.5 py-3 text-sm text-neutral-800 focus:outline-none focus:border-primary focus:bg-white transition-all font-sans"
                        >
                      ...
                      <div className="space-y-1.5">
                        <label className="text-xs font-sans uppercase tracking-wider text-neutral-700 block font-bold">Experience (Yrs)</label>
                        <select 
                    ...
                    <div className="space-y-1.5">
                      <label className="text-xs font-sans uppercase tracking-wider text-neutral-700 block font-bold">CV / Resume (PDF / Doc)</label>
                      <div className="border border-dashed border-neutral-300 hover:border-primary rounded-xl p-4 bg-neutral-50 hover:bg-neutral-100/50 transition-all text-center relative cursor-pointer">
                        <input 
                          type="file" 
                          accept=".pdf,.doc,.docx"
                    ...
                    <div className="space-y-1.5">
                      <label className="text-xs font-sans uppercase tracking-wider text-neutral-700 block font-bold">Statement of Intent</label>
                      <textarea 
                        rows={3}
                  ...
                  <button 
                    type="submit"
                    className="w-full bg-rust hover:bg-rust-dark text-white font-bold py-4 rounded-xl text-sm uppercase tracking-wider transition-colors flex items-center justify-center gap-2 shadow-md cursor-pointer"
                  >
                    Submit Application
