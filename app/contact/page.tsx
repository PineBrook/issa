...
import { getOfficeLocations, getFaqs, getSiteSettings } from '@/lib/site-cms';
...
export const metadata: Metadata = {
  ...
  openGraph: {
    type: 'website',
    url: 'https://issafoundation.co.in/contact',
...
export default async function ContactPage() {
  const [offices, faqs, settings] = await Promise.all([
    getOfficeLocations().catch(() => []),
    getFaqs('contact').catch(() => []),
  ...
  return (
    <>
      ...
      <ContactView offices={offices} faqs={faqs} settings={settings} />
    </>
  );
}
